...
/// <summary>
/// Interview transcript: INTERVIEW RECORD banner, header strip with
/// recording reference + duration. Section bodies (most often a
/// <c>transcript</c> kind) are rendered with speaker column + timestamp
/// metadata via <see cref="DocumentBlocks.RenderTranscript"/>.
/// </summary>
public sealed class InterviewTranscriptLayout : IDocumentLayout
{
    public string Layout => "InterviewTranscript";
    ...
    private static void BuildHeader(IContainer container, EvidenceDocument doc)
    {
        container.Column(col =>
        {
            col.Item().Background("#0F2C5E").Padding(6).Row(r =>
            {
                ...
                {
                    c.Item().Text(DocumentChrome.Get(ChromeKey.InterviewRecord, doc.Language))
